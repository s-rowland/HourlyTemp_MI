...
############################3
analyze_hourlytemp_special_rh <- function(CaseType,  NumHr,  AveWindow,  LagConstraint,  ERConstraint, SubPopVar, SubPop, prim, RHADJUST){
...
#ERConstraint <- "4dfevenknots"; SubPopVar <- "fullpop"; SubPop <- "fullpop"; prim <- "primary"
  # RHADJUST <- "RHDLNM"
  ...
  ModelName <- paste0("hourlytemp", "_",CaseType, "_", NumHr, "_", AveWindow, "_Lag", LagConstraint, "_ER", ERConstraint, "_",SubPopVar, "_", SubPop, "_", prim, "_",RHADJUST )
...
df.t <- df %>% select(starts_with("tLag_"),meanr, EventID, Case, TimetoEvent)
...
if(RHADJUST == "NORH"){
mod  <- coxph(Surv(TimetoEvent, Case) ~ cb.hrtemp + strata(EventID),
                 method = "exact", 
                 na.action = na.omit, 
                 data = df)
}
if(RHADJUST == "RHDLNM"){
  ...
  # 4c Isolate exposure variables
  df.rh <- df %>% select(starts_with("rLag_"),meanr, EventID, Case, TimetoEvent)
  ...
  # 4f Create crossbasis
  if(str_detect(LagConstraint, "evenknots")){
    cb.hrrh <- crossbasis(exposure.profiles, lag=c(0,NumHr-1),
                            argvar=list(fun="ns", df = 4),
                            arglag=list(fun="ns", df = 6))
  }
  
  
  mod  <- coxph(Surv(TimetoEvent, Case) ~ cb.hrtemp +cb.hrrh + strata(EventID),
                method = "exact", 
                na.action = na.omit, 
                data = df)
}
